...
const timer= {fontSize: 40, position: "relative", left: "35%", top: "37%"};
...
function Clock({secondTime}){
    ...
    //         return minute
    // }
    var minute = new Date(secondTime*1000).getMinutes();
    var second = new Date(secondTime*1000).getSeconds();
    // var milli = new Date(secondTime*1000).getMilliseconds();
    ...
    return(
            <div className="rounded-circle m-auto" style={circle}>
                <span style={timer} className="text-center">{CalMinute}:{CalSecond}</span>
            </div>
    )
}
